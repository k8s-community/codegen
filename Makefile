...
CODEGEN_LOCAL_HOST?=0.0.0.0
CODEGEN_LOCAL_PORT?=8080
CODEGEN_LOG_LEVEL?=0

# Namespace: dev, prod, release, app, username ...
...
run: build
	@echo "+ $@"
	@docker run --name ${CONTAINER_NAME} -p ${CODEGEN_LOCAL_PORT}:${CODEGEN_LOCAL_PORT} \
		-e "CODEGEN_LOCAL_HOST=${CODEGEN_LOCAL_HOST}" \
		-e "CODEGEN_LOCAL_PORT=${CODEGEN_LOCAL_PORT}" \
		-e "CODEGEN_LOG_LEVEL=${CODEGEN_LOG_LEVEL}" \
